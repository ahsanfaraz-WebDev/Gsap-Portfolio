...
export default function AboutSection() {
  ...
  useEffect(() => {
    ...
    // Initialize the hover animation for expertise cards
    if (skillsRef.current && !CSS.supports('anchor-name: --anchor')) {
      ...
      const sync = () => {
        if (current) {
          ...
          LIST.style.setProperty('--top', (BOUNDS.top - listBounds.top).toString())
          LIST.style.setProperty('--right', (BOUNDS.right - listBounds.left).toString())
          LIST.style.setProperty('--bottom', (BOUNDS.bottom - listBounds.top).toString())
          LIST.style.setProperty('--left', (BOUNDS.left - listBounds.left).toString())
        }
      }

      const UPDATE = ({ x, y }: { x: number, y: number }) => {
        const element = document.elementFromPoint(x, y)
        const ARTICLE = element?.closest('li')?.querySelector('article') as HTMLElement
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        {/* Professional Skills Cards with Hover Animation */}
        <div className="mt-20">
          <h4 
      ...
      <style jsx global>{`
        ...
        .expertise-grid[data-enhanced]:hover {
          --active: 1;
        }

        .expertise-grid[data-enhanced]::after {
          opacity: var(--active, 0);
          transition: opacity 0.2s, inset 0.2s 0.2s;
        }

        .expertise-grid[data-enhanced]:hover::after {
          transition: opacity 0.2s 0.2s, inset 0.2s;
        }

        @supports(anchor-name: --anchor) {
          .expertise-item:hover .expertise-article { 
            anchor-name: --active; 
          }
          
          :root:has(.expertise-item:hover) { 
            --anchor: --active; 
          }
          ...
          .expertise-grid:has(.expertise-item:hover) {
            --active: 1;
          }
        ...
        .expertise-grid:not([data-enhanced]) .expertise-article::after {
          content: "";
          position: absolute;
          inset: 0;
          z-index: -2;
        ...
        .expertise-article::before {
          ...
          -webkit-mask: linear-gradient(-35deg, hsl(0 0% 2%) 0%, transparent 45%);
          z-index: -1;
          opacity: var(--li-active, 0);
          transition: opacity 0.2s;
        ...
        .expertise-article svg {
          fill: none;
          stroke: currentColor;
        }

        .expertise-article svg path {
          stroke-width: 0.75;
